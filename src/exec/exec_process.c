...
static void	ft_exec_buildin(t_process *process, char ***envv)
{
	int	dup_STDIN;
	int	dup_STDOUT;
...
void	ft_execute_process(t_process *process, char ***envv)
{
	if (process->is_buildin)
	{
		ft_exec_buildin(process, envv);
		return ;
	}
	process->pid = fork();
	if (process->pid == -1)
	...
	if (process->pid == 0)
	{
		...
		execve(process->cmd, process->args, *envv);
		exit(1);
	}
	ft_close_fd(&process->pipe_fd_in[PIPE_READ]);
	ft_close_fd(&process->pipe_fd_in[PIPE_WRITE]);
